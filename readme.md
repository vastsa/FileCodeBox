...
本仓库在 FileCodeBox 基础上增加 [寄件码功能](./docs/guide/delivery.md)：管理员预先生成口令，临时访客凭码投递到指定目录；关闭游客自由上传后仍可收件。寄件码只授权上传，后台可管理有效期、次数和已收文件。

- 访客投递：`/delivery`
- 寄件管理：`/delivery/admin`（使用本站管理员密码）
- 后端在本仓库，2024 原生寄件页面在独立前端仓库；启动时自动执行迁移 008–010。旧主题保留独立页面回退。使用包含前后端改动的自建程序；下方上游预构建镜像不包含此扩展。
