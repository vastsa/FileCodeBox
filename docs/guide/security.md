...
::: danger 重要安全警告
FileCodeBox 首次启动时不会生成默认管理员密码。请在浏览器中打开站点，按初始化页面设置管理员密码，并妥善保存。
:::
...
`admin_token` 使用密码哈希存储。不要向数据库写入明文密码，否则管理员登录将失效。
...
::: warning 注意
默认配置允许每个 IP 在 1 分钟内最多出现 10 次错误。公开服务可根据访问规模适当收紧，并避免误伤共享出口用户。
:::
